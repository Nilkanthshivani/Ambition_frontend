...
class _PassengerHistoryPageState extends State<PassengerHistoryPage> {
  ...
  void initState() {
    ...
    _rideRequestBloc.add(GetClosedRideRequestsByUserIdEvent());
  }
      ...
      body: BlocConsumer<RideRequestBloc, RideRequestState>(
        ...
        builder: (context, state) {
          if (state is RideRequestError) {
            ...
            final rideRequests = state.rideRequests;
          ...
          Padding(
            ...
            child: Column(
              ...
              children: [
                ...
                const SizedBox(height: 16),
                _buildSectionTitle(context, 'Fare Breakdown'),
                _buildDetailRow('Initial Service Fee',
                    '\$${rideRequest.fare.vehicleInitialServiceFee.toStringAsFixed(2)}'),
                _buildDetailRow('Vehicle Event Service Fee',
                    '\$${rideRequest.fare.vehicleServiceFee.toStringAsFixed(2)}'),
                _buildDetailRow('Vehicle Time Fare',
                    '\$${rideRequest.fare.vehicleTimeFare.toStringAsFixed(2)}'),
                _buildDetailRow('Vehicle Item Based Pricing',
                    '\$${rideRequest.fare.vehicleItemBasedPricing.toStringAsFixed(2)}'),
                _buildDetailRow('Car Time Fare',
                    '\$${rideRequest.fare.carTimeFare.toStringAsFixed(2)}'),
                _buildDetailRow('Helpers Charge',
                    '\$${rideRequest.fare.helpersCharge.toStringAsFixed(2)}'),
                _buildDetailRow('Congestion Charge',
                    '\$${rideRequest.fare.congestionCharge.toStringAsFixed(2)}'),
                _buildDetailRow('Surcharge',
                    '\$${rideRequest.fare.surcharge.toStringAsFixed(2)}'),
                ...
                if (rideRequest
                    .requirements.specialRequirements.isNotEmpty) ...[
                  Text.rich(
                    TextSpan(
                      children: [
                        const TextSpan(
                        ...
                        TextSpan(
                          text: rideRequest.requirements.specialRequirements,
                        ),
                      ],
                    ),
                  )
